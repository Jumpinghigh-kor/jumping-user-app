...
const PostDetail = () => {
  ...
  useEffect(() => {
    const runSideEffect = async () => {
      ...
      try {
        const allSendYnRaw = (post as any).all_send_yn;
        const readYnRaw = post.read_yn as any;
        const allSendYn = (allSendYnRaw ?? '').toString().trim().toUpperCase();
        const readYn = (readYnRaw ?? '').toString().trim().toUpperCase();

        // ALL 발송(Y):
        // - read_yn 비어있음(null/undefined/empty) → insert (개인 읽음 레코드 생성)
        // - read_yn === 'N' → update to 'Y'
        if (allSendYn === 'Y') {
          if (!readYn) {
            await insertMemberPostApp({ post_app_id: post.post_app_id, mem_id: memberInfo?.mem_id });
            return;
          }
          if (readYn === 'N' && post.member_post_app_id) {
