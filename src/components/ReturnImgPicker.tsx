...
const ReturnImgPicker: React.FC<ReturnImgPickerProps> = ({
  maxImages = 3,
  onImagesSelected,
  onFileIdsChange,
  onDeletedReturnImgIds,
  memberId,
  returnAppId,
  disabled = false,
  hideTitle = false,
  route = 'ShoppingReturn',
}) => {
  const [selectedImages, setSelectedImages] = useState<Asset[]>([]);
  const [fileIds, setFileIds] = useState<number[]>([]);
  const [filePaths, setFilePaths] = useState<string[]>([]);
  const [existingReturnImgIds, setExistingReturnImgIds] = useState<number[]>([]);
  const [deletedReturnImgIds, setDeletedReturnImgIds] = useState<number[]>([]);
  ...
  const loadExistingImages = async () => {
    try {
      const response = await getMemberReturnAppList({ return_app_id: returnAppId });
      if (response.success && response.data) {
        ...
        const existingImages: Asset[] = response.data.map((img: any) => {
          ...
          if (img.file_name) {
            const filePath = `return/${img.file_name}`;
            const { data } = supabase.storage.from('return').getPublicUrl(filePath);
        ...
        setExistingReturnImgIds(response.data.map((img: any) => img.return_app_img_id));
      }
    } catch (error) {
  ...
  // Supabase 리뷰 이미지 업로드
  const uploadReturnImageToSupabase = async (base64Image: string): Promise<string | null> => {
    try {
      ...
      const fileName = `return_${memberId}_${dateStr}.jpg`;
      const filePath = `return/${fileName}`;
    ...
    try {
      ...
      if (result.assets && result.assets.length > 0) {
        ...
        if (asset.base64 && memberId) {
          const fileName = await uploadReturnImageToSupabase(asset.base64);
          if (fileName) {
            try {
              const response = await insertCommonFile({
                file_name: fileName,
                file_path: '/return',
                file_division: 'return',
              ...
              if (response.success && response?.file_id) {
                ...
                newFilePaths[index] = `return/${fileName}`;
  ...
  const updateImage = (index: number, asset: Asset) => {
    ...
    if (selectedImages[index] && existingReturnImgIds[index]) {
      const existingImgId = existingReturnImgIds[index];
      const newDeletedIds = [...deletedReturnImgIds, existingImgId];
      setDeletedReturnImgIds(newDeletedIds);
      if (onDeletedReturnImgIds) {
        onDeletedReturnImgIds(newDeletedIds);
      }
      
      // existingReturnImgIds에서 해당 인덱스 제거 (새 이미지 file_id가 fileIds에 담기도록)
      const newExistingIds = [...existingReturnImgIds];
      newExistingIds[index] = null;
      setExistingReturnImgIds(newExistingIds);
    }
    
    // 새로 업로드한 이미지가 있고 교체하는 경우 기존 fileId 제거
    if (selectedImages[index] && fileIds[index] && !existingReturnImgIds[index]) {
      const newFileIds = [...fileIds];
...
export default ReturnImgPicker; 